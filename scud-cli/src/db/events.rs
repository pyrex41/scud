    ...
    let (kind, success, duration_ms, tool_name, file_path, dependency_id, reason, data) =
        match &event.event {
            ...
            EventKind::WaveStarted {
                ...
                None,
                None,
                Some(serde_json::json!({"wave_number": wave_number, "task_count": task_count}).to_string()),
            ),
...
pub fn get_events_for_session(conn: &Connection, session_id: &str) -> Result<Vec<AgentEvent>> {
    ...
    let events = stmt.query_map(params![session_id], |row| {
        ...
        let event = match kind.as_str() {
            ...
            "wave_started" => {
                let parsed = data.as_ref().and_then(|d| serde_json::from_str::<serde_json::Value>(d).ok());
                EventKind::WaveStarted {
                    wave_number: parsed.as_ref().and_then(|v| v.get("wave_number").and_then(|n| n.as_u64())).unwrap_or(0) as usize,
                    task_count: parsed.as_ref().and_then(|v| v.get("task_count").and_then(|n| n.as_u64())).unwrap_or(0) as usize,
                }
            }
            "wave_completed" => {
                let parsed = data.as_ref().and_then(|d| serde_json::from_str::<serde_json::Value>(d).ok());
            ...
            "repair_started" => {
                ...
                EventKind::RepairStarted {
                    attempt: parsed.as_ref().and_then(|v| v.get("attempt").and_then(|n| n.as_u64())).unwrap_or(0) as usize,
                    task_ids: parsed
                        .as_ref()
