...
- [ ] **[CURRENT]** Phase 2: Messages + Handlers — New Message variants and update handlers in `main.rs`
  - Why: Wires state changes to UI events; foundation for all interactive controls
  - Add SetHarness, SetModel, SetRoundSize, SetLaunchTag, SetAgentType, TagsLoaded, AgentsLoaded, SpawnTask
  - Update StartSwarmHeadless handler to use launch_config.model
  - Send LoadAvailableTags/LoadAvailableAgents on init
  - Spec: FR-4 through FR-7

## Backlog (Prioritized)

1. [ ] Phase 5: Bridge — Model plumbing + tag/agent loading in `scud_bridge.rs`
   - Why: Bridge must support new commands before UI can use them; moved before Phase 3 since the view needs working bridge commands
   - Add model field to StartSwarmHeadless, RunTaskHeadless, RunTask, StartSwarm commands
   - Add LoadAvailableTags, LoadAvailableAgents commands
   - Add TagsLoaded, AgentsLoaded events
   - Pass model through to runner.start() calls
   - Spec: FR-15 through FR-19

2. [ ] Phase 3: Agents View — Config panel rewrite of `views/agents.rs`
...
3. [ ] Phase 4: Waves View — Spawn button per task in `views/waves.rs`
...
4. [ ] Phase 6: Tests — Update and add tests
