...
3. **Investigate existing code** - don't assume not implemented. Search `descartes-gui/src/` thoroughly
...
996. **Follow project conventions**: See AGENTS.md for iced patterns
...
- Sequential: `cargo fmt -p descartes-gui --check && cargo clippy -p descartes-gui -- -D warnings && cargo test -p descartes-gui`
- "Validate: run format check, clippy, then tests. Stop on first failure."
...
```bash
cargo fmt -p descartes-gui --check && cargo clippy -p descartes-gui -- -D warnings && cargo test -p descartes-gui
```
...
Spec: specs/gui-config-controls.md"
```
